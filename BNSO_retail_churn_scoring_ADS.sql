...
left join 
day_since_last_service_bnso_retail_churn_scoring
b on trim(a.fca_id)=trim(b.fca_id) 
left join 
leads_submit_last90_days_bnso_retail_churn_scoring
c on trim(a.fca_id)=trim(c.fca_id)
left join 
avg_vin_ownership_tenure_bnso_retail_churn_scoring
d on trim(a.fca_id)=trim(d.fca_id) 
left join 
stellantis_tenure_bnso_retail_churn_scoring
e on trim(a.fca_id)=trim(e.fca_id)
left join 
nVehicles_purchased_bnso_retail_churn_scoring
f on trim(a.fca_id)=trim(f.fca_id)
left join 
avg_serv_spend_bnso_retail_churn_scoring
g on trim(a.fca_id)=trim(g.fca_id) 
left join 
serv_tat_bnso_retail_churn_scoring
h on trim(a.fca_id)=trim(h.fca_id) 
left join 
total_spend_total_serv_bnso_retail_churn_scoring
i on trim(a.fca_id)=trim(i.fca_id) 
left join
ttl_act_new_vhcl_own_last_5_yr_bnso_retail_churn_scoring
j on trim(a.fca_id)=trim(j.fca_id)
left join 
disp_ratio_bnso_retail_churn_scoring
k on trim(a.fca_id)=trim(k.fca_id)
left join 
retail_recall_open_sseo_std_bnso_retail_churn_scoring
l on trim(a.fca_id)=trim(l.fca_id) 
left join 
recall_type_safety_bnso_retail_churn_scoring
m on trim(a.fca_id)=trim(m.fca_id) 
left join 
recall_type_federal_emissn_bnso_retail_churn_scoring
n on trim(a.fca_id)=trim(n.fca_id) 
left join 
recall_type_customer_satisfaction_bnso_retail_churn_scoring
o on trim(a.fca_id)=trim(o.fca_id) 
left join 
nVehicles_purchased_which_were_recall_bnso_retail_churn_scoring
p on trim(a.fca_id)=trim(p.fca_id)
left join 
nDaysSince_last_vehcile_purchase_bnso_retail_churn_scoring
q on trim(a.fca_id)=trim(q.fca_id)
left join 
nvehicles_brand_jeep_ram_dodge_active_std_bnso_retail_churn_scoring
r on trim(a.fca_id)=trim(r.fca_id)
left join 
no_of_Vins_owned_in_household_bnso_retail_churn_scoring
s on trim(a.fca_id)=trim(s.fca_id)
left join 
in_market_prob_bnso_retail_churn_scoring
t on trim(a.fca_id)=trim(t.fca_id)
left join 
num_vins_disposed_household_bnso_retail_churn_scoring
