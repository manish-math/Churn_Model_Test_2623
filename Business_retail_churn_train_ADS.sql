...
a.vin_disp_30,a.churn_30 
from ${business_retail_base} a 
left join leads_submit_last90_days_businsess_retail_train	b
on a.fca_id=b.fca_id		
left join 	stellantis_tenure_business_retail_churn_train	c
on a.fca_id=c.fca_id 		
left join 	serv_tat_business_retail_churn_train	d
on a.fca_id=d.fca_id and a.vin=d.vin		
left join 	num_vins_disposed_business_retail_churn_train	e
on a.fca_id=e.fca_id 		
left join 	days_since_last_service_business_retail_churn_train	f
on a.fca_id=f.fca_id and a.vin=f.vin		
left join 	mileage_recorded_from_last_service_for_active_owned_std_business_retail_churn_train	g
on a.fca_id=g.fca_id and a.vin=g.vin		
left join 	nvehicles_brand_jeep_ram_dodge_active_std_business_retail_churn_train	h
on a.fca_id=h.fca_id		
left join 	nVehicles_purchased_which_were_recall_business_retail_churn_train	i
on a.fca_id=i.fca_id		
left join 	nVehicles_purchased_ft_business_retail_churn_train	j
on a.fca_id=j.fca_id		
left join 	nservices_active_vehicle_business_retail_churn_train	k
on a.fca_id=k.fca_id and a.vin=k.vin
left join 	total_spend_total_serv_business_retail_churn_train	l
on a.fca_id=l.fca_id and a.vin=l.vin	
left join 	single_multi_veh_flag_business_retail_churn_train	m
on a.fca_id=m.fca_id		
left join   avg_days_between_disposals_business_retail_churn_train n
on a.fca_id=n.fca_id
left join   avg_days_between_purchases_business_retail_churn_train o
