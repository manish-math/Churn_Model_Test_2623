...
from ${owner}
where
(date(d_purchase) BETWEEN (date('${train_ref_date}') - INTERVAL '5' year) and date('${train_ref_date}'))
and c_cntry_srce = '1'
...
-- union 
select fca_id,status,'Retail' as retail_lease_flag from cpov_base
union 
select fca_id,status,'Retail' as retail_lease_flag from buso_base
)

SELECT fca_id,status,retail_lease_flag,i_hshld_id_curr,vin,l_new_vhcl,c_sales_typ_orig,d_purchase,true_disp, x_vhcl_make, x_nmplt
from (
SELECT b.fca_id,status,retail_lease_flag,l_new_vhcl,i_hshld_id_curr , x_vhcl_make, x_nmplt,i_vin_first_9||i_vin_last_8 as vin,
LEAST(COALESCE(d_vhcl_dspsl,cast(CURRENT_DATE as VARCHAR)),
COALESCE(d_leas_end,cast(CURRENT_DATE as VARCHAR))
) as true_disp,l_cert_usd_vhcl,d_purchase,c_sales_typ_orig,
