...
(
    ...
    from 
      ...
      from ${stg_serv_rec}
      where i_vin in (select distinct vin from ${bnso_lease_train_base}) 
      -- c_ro_reas in ('1','3','-1')
      and 
...
and trim(c_sales_typ_orig) in ('L')
  and fca_id in (select fca_id from ${bnso_lease_train_base} where status like 'BNSO' and trim(retail_lease_flag) = 'Lease')
...
and trim(retail_lease_flag) = 'Lease'
)
