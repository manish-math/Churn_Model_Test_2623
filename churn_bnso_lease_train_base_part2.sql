...
from (
  ...
  from ${owner}
  where
  fca_id in (select distinct fca_id from churn_bnso_lease_base_part1)
  and date(d_purchase) BETWEEN (date('${train_ref_date}') - INTERVAL '60' day) AND (date('${train_ref_date}') + INTERVAL '120' day)
  and c_cntry_srce = '1'
...
from date_vhcl_dispsl a inner join churn_bnso_lease_base_part1 b 
...
from churn_bnso_lease_base_part1
where
...
disposalin_ref_90 as
(
select distinct fca_id,1 as churn_90 
from churn_bnso_lease_base_part1
where
date(true_disp) BETWEEN (date('${train_ref_date}') + INTERVAL '1' day) and (date('${train_ref_date}') + INTERVAL '90' day)
and fca_id not in (select distinct fca_id from exclusion_repurchasers)
-- GROUP by 2
),
...
final_disp as (
SELECT distinct fca_id,i_hshld_id_curr,status,vin,d_purchase,true_disp,l_new_vhcl,c_sales_typ_orig,retail_lease_flag,
churn_30,churn_90
from (
    select distinct a.*,DENSE_RANK()over(PARTITION by vin ORDER by d_purchase desc) as p_rank,
    COALESCE(churn_30,0) churn_30,COALESCE(churn_90,0) churn_90
    from churn_bnso_lease_base_part1 a 
    left join disposalin_ref_30 b 
    on a.fca_id=b.fca_id
    left join disposalin_ref_90 c 
...
vin_disp_base as (
SELECT *,1 as vin_disp_30 from base2
where churn_30=1 
and date(true_disp) BETWEEN (date('${train_ref_date}') + INTERVAL '1' day) and (date('${train_ref_date}') + INTERVAL '30' day)
),

vin_disp_base_90 as (
SELECT *,1 as vin_disp_90 from base2
where churn_90=1 
and date(true_disp) BETWEEN (date('${train_ref_date}') + INTERVAL '1' day) and (date('${train_ref_date}') + INTERVAL '90' day)
)

SELECT a.*,COALESCE(vin_disp_30,0) vin_disp_30,COALESCE(vin_disp_90,0) vin_disp_90
from base2 
a left join vin_disp_base b 
on a.fca_id=b.fca_id
and a.vin=b.vin 
left join vin_disp_base_90 c
on a.fca_id=c.fca_id
and a.vin=c.vin 
