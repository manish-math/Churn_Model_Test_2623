...
(
    ...
    from 
      ...
      from ${stg_serv_rec}
      where i_vin in (select distinct vin from ${buso_retail_base}) and
      -- c_ro_reas in ('1','3','-1') 
      ...
      (
        ...
        where i_vin  in
              (select i_vin  
              from ${stg_serv_rec}
              where i_vin in (select distinct vin from ${buso_retail_base})
              -- c_ro_reas in ('1','3','-1')
              and
               cast(d_ro_invc as date) between cast('${train_ref_date}' as date) - interval '10' year and cast('${train_ref_date}' as date)
              group by i_vin)
...
and trim(c_sales_typ_orig) in ('1')
  and fca_id in (select fca_id from ${buso_retail_base})
...
where trim(c_sales_typ_orig) in ('1')
)
