...
select fca_id,count(distinct i_vin_first_9||i_vin_last_8) as Vins_owned_within_household
from ${owner}
where fca_id in (select fca_id from ${business_lease_base})
and c_cntry_srce = '1'
...
fca_id_lvl as (
select distinct a.fca_id , Vins_owned_within_household
