...
,COALESCE(recall_type_safety,0) as recall_type_safety, d_recal_lnch
...
get_fca_lvl as (
select fca_id,vin,recall_type_safety,d_recal_lnch
FROM recall_consmr_frm_base
)

select distinct fca_id,vin,max(recall_type_safety) as recall_type_safety 
from get_fca_lvl
group by 1,2